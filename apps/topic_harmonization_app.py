...
WORKBOOK_PATH = get_workbook_path(APP_DIR)
BACKUP_DIR = APP_DIR / "backups_topic_harmonization"

st.set_page_config(page_title="Topic Harmonization", layout="wide")
...
st.markdown(
    ...
    .coding-box {border: 1px solid #ddd; border-radius: 6px; padding: 0.75rem; min-height: 8rem; background: #fafafa;}
    </style>
    """,
...
backup_key = "backup_path_harmonization"
if st.session_state.get(backup_key):
...
with save_col:
    if st.button("Save harmonization", type="primary"):
        ...
        save_private_workbook(df, WORKBOOK_PATH, BACKUP_DIR, st.session_state, "harmonization")
        st.cache_data.clear()
        st.success(f"Saved in {HARMONIZED_COL}.")
        st.rerun()

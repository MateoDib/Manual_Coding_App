...
BACKUP_DIR = BASE_DIR / "backups_topic_coding"
...
def parse_topic_components(token: str) -> Tuple[str, List[str], Optional[str]]:
    ...
    Supported internal or manually typed forms include:
        base
        base[scope]
        base[subtopic_1,subtopic_2] is not supported because square brackets are scopes
...
def make_topic_token(base: str, subtopics: Optional[List[str]] = None, scope: Optional[str] = None) -> str:
    ...
    Examples:
        ...
        make_topic_token("unreadiness", [], "modest_household, company (agricultural_sector, sme)")
...
def normalize_dag_token(token: str) -> str:
    """
    Normalize a token in the coding sequence.
...
def raw_tokenize_dag_text(text: str) -> List[str]:
    ...
    This tokenizer is intentionally permissive so that it can parse:
        ...
        A(X,Y)[self] --> B
        A[self] (X, Y) --> B
        A[modest_household, company (agricultural_sector, sme)] --> B
...
def get_all_subtopics_from_df(df: pd.DataFrame) -> List[str]:
    """Build a subtopic dictionary from all coder columns."""
    subtopics: List[str] = []
...
def get_all_scope_qualifiers_from_df(df: pd.DataFrame) -> List[str]:
    """Build a scope qualifier dictionary from all coder columns."""
    scopes: List[str] = []
    for col in CODER_COLS.values():
        if col in df.columns:
            for value in df[col]:
...
def get_subtopic_options(
    ...
    - substantive tokens already present in the current coding sequence;
...
def get_all_topics_from_df(df: pd.DataFrame) -> List[str]:
    """Build a substantive topic dictionary from all coder columns."""
    topics: List[str] = []
    for col in CODER_COLS.values():
        if col in df.columns:
            for value in df[col]:
...
def get_candidate_topics_for_row(row: pd.Series, current_coder_col: str) -> List[str]:
    """
    Return unique substantive topics proposed in the current row by any coder column.
    ...
    if current_coder_col in row.index:
        topics.extend(get_substantive_topics(split_topics(row[current_coder_col])))
...
def load_main_file() -> pd.DataFrame:
    """Load the Excel file and create coder columns if necessary."""
    ...
    for col in required_cols + list(CODER_COLS.values()):
...
def make_backup_once() -> None:
    ...
    backup_file = BACKUP_DIR / f"Topics_Coding_backup_{timestamp}.xlsx"
...
def init_row_state(row_index: int, coder_col: str, current_tokens: List[str]) -> None:
    ensure_session_defaults()
    if (
        st.session_state["active_row_index"] != row_index
        or st.session_state["active_coder_col"] != coder_col
    ):
        st.session_state["active_row_index"] = row_index
        st.session_state["active_coder_col"] = coder_col
        st.session_state["coding_sequence"] = current_tokens.copy()
...
def push_history() -> None:
    current_sequence = st.session_state.get("coding_sequence", []).copy()
    history = st.session_state.get("coding_history", [])
    if history and history[-1] == current_sequence:
        return
    history.append(current_sequence)
    if len(history) > MAX_HISTORY_SIZE:
        history = history[-MAX_HISTORY_SIZE:]
    st.session_state["coding_history"] = history
...
def undo_last_change() -> None:
    ...
    if not history:
        return
    st.session_state["coding_sequence"] = history.pop()
    st.session_state["coding_history"] = history
    refresh_topic_input()
    refresh_subtopic_input()
    refresh_scope_input()
...
def add_token_to_sequence(token: str) -> None:
    ...
    if token:
        push_history()
        st.session_state["coding_sequence"].append(token)
        refresh_topic_input()
        refresh_sortable_list()
...
def add_topics_to_sequence(topics: List[str]) -> None:
    ...
    if clean_tokens:
        push_history()
        st.session_state["coding_sequence"].extend(clean_tokens)
        refresh_topic_input()
        refresh_sortable_list()
...
def replace_sequence(tokens: List[str]) -> None:
    clean_tokens = [normalize_dag_token(token) for token in tokens if normalize_dag_token(token)]
    if clean_tokens != st.session_state.get("coding_sequence", []):
        push_history()
        st.session_state["coding_sequence"] = clean_tokens
        refresh_topic_input()
        refresh_subtopic_input()
        refresh_scope_input()
        refresh_sortable_list()
...
def remove_topic_at_position(position: int) -> None:
    sequence = st.session_state.get("coding_sequence", [])
    if 0 <= position < len(sequence):
        push_history()
        sequence.pop(position)
        st.session_state["coding_sequence"] = sequence
        refresh_sortable_list()


def clear_sequence() -> None:
    if st.session_state.get("coding_sequence", []):
        push_history()
        st.session_state["coding_sequence"] = []
        refresh_sortable_list()


def update_sequence_after_sorting(new_sequence: List[str]) -> None:
    old_sequence = st.session_state.get("coding_sequence", [])
    if new_sequence != old_sequence:
        push_history()
        st.session_state["coding_sequence"] = new_sequence
        refresh_sortable_list()
...
def add_subtopics_to_last_topic(subtopics_text: str) -> bool:
    ...
    return add_subtopics_to_topic_at_index(
        find_last_substantive_topic_index(st.session_state.get("coding_sequence", [])),
        subtopics_text,
    )
...
st.set_page_config(
    page_title="Interview topic coding",
    layout="wide",
)
...
st.title("Interview topic coding")
...
selected_coder_name = st.sidebar.selectbox(
    "Coder",
    options=list(CODER_COLS.keys()),
    index=0,
    help="Choose the coder column that will be edited and saved.",
)
selected_coder_col = CODER_COLS[selected_coder_name]

st.sidebar.caption(f"Current output column: `{selected_coder_col}`")
...
show_uncoded_only = st.sidebar.checkbox(
    "Rows without coding only for selected coder",
    value=False,
)

if show_uncoded_only:
    uncoded_indices = df[df[selected_coder_col].apply(is_uncoded)].index.tolist()
...
n_coded = df[selected_coder_col].fillna("").astype(str).str.strip().ne("").sum()
st.sidebar.write(f"Rows: **{n_rows}**")
st.sidebar.write(f"Coded rows for {selected_coder_name}: **{n_coded}**")
st.sidebar.write(f"Remaining: **{n_rows - n_coded}**")
st.sidebar.write(f"Unique substantive topics: **{len(all_topics)}**")
st.sidebar.write(f"Scope qualifiers: **{len(all_scope_qualifiers)}**")
st.sidebar.write(f"Undo states: **{len(st.session_state.get('coding_history', []))}**")
...
with st.sidebar.expander("DAG syntax reminder", expanded=False):
    st.markdown(
        ...
        `A[scope_1, group (subgroup_1, subgroup_2)]` : composite scope.  
...
current_tokens = split_topics(row[selected_coder_col])
init_row_state(row_index, selected_coder_col, current_tokens)

st.subheader(f"Row {row_index} — {selected_coder_name}")

progress = n_coded / n_rows if n_rows else 0
st.progress(progress, text=f"{n_coded}/{n_rows} rows coded for {selected_coder_name}")
...
candidate_topics = get_candidate_topics_for_row(row, selected_coder_col)

with st.expander("Candidate topics for this row", expanded=True):
    st.markdown("#### DAG operators")
    render_token_buttons(DAG_OPERATORS, key_prefix=f"dag_operator_{row_index}_{selected_coder_col}", columns_count=8)

    st.markdown("#### Required ending nodes")
    render_token_buttons(REQUIRED_ENDING_NODES, key_prefix=f"ending_node_{row_index}_{selected_coder_col}", columns_count=3)

    st.markdown("#### Candidate substantive topics")
    if candidate_topics:
        render_token_buttons(candidate_topics, key_prefix=f"candidate_{row_index}_{selected_coder_col}", columns_count=6)
    else:
        st.info("No candidate substantive topic for this row yet.")
...
if substantive_indices:
    ...
    selected_subtopic_target_index = st.selectbox(
        ...
        key=f"subtopic_target_{row_index}_{selected_coder_col}_{st.session_state['sortable_version']}",
...
subtopic_choice_key = f"subtopic_choice_{row_index}_{selected_coder_col}_{st.session_state['subtopic_input_version']}"
...
if substantive_indices:
    ...
    selected_scope_target_index = st.selectbox(
        ...
        format_func=lambda idx: f"{idx + 1}. {format_dag_token(sequence[idx])}",
        key=f"scope_target_{row_index}_{selected_coder_col}_{st.session_state['sortable_version']}",
        help=(
...
scope_choice_key = f"scope_choice_{row_index}_{selected_coder_col}_{st.session_state['scope_input_version']}"
...
full_scope_text = st.text_input(
    ...
    key=f"full_scope_text_{row_index}_{selected_coder_col}_{st.session_state['scope_input_version']}",
    disabled=not bool(substantive_indices),
    help=(
...
manual_text = st.text_area(
    "Or paste/edit the full coding sequence",
    ...
    key=f"manual_text_{row_index}_{selected_coder_col}_{st.session_state['sortable_version']}",
)

col_apply_manual, col_clear_manual = st.columns([1, 4])
with col_apply_manual:
    if st.button("Apply text", key=f"apply_manual_{row_index}_{selected_coder_col}"):
        replace_sequence(split_topics(manual_text))
        st.rerun()

final_sequence = st.session_state.get("coding_sequence", [])
final_topics_string = join_dag_tokens(final_sequence)
validation_warnings = validate_dag_sequence(final_sequence)

st.markdown("#### Final coding")
...
with col_undo:
    if st.button(
        "Undo",
        disabled=len(st.session_state.get("coding_history", [])) == 0,
    ):
        undo_last_change()
        st.rerun()
...
with col_save:
    if st.button("Save", type="primary"):
        df.at[row_index, selected_coder_col] = final_topics_string
        try:
            save_main_file(df)
...
with st.expander("Coding overview"):
...
with st.expander("Substantive topic dictionary"):
    dictionary_topics = sorted(set(
        ...
        + candidate_topics
        + REQUIRED_ENDING_NODES
        + get_substantive_topics(st.session_state.get("coding_sequence", []))
    ))
    st.dataframe(
...
with st.expander("DAG coding grammar"):
    st.markdown(
        """
        The coding is treated as an ordered token sequence.
